...
# usage
# - 依存モジュールあり (cpnam JSON::XS Getopt::Kingping 等する)
# - minibun.pl -n <名前> -d <20XX-YY-ZZ> <URL1> <URL2> ... <URLn> → 詳細は --help
...
my $name      = $kingpin->flag('name', 'presenter name')->short('n')->string->default('name');
...
sub _get_html {
  ...
  my $res = $http->get($url);
  die "cannot access $url" unless $res->{success};
